...
def parse_template(token_list, upto, parent, parent_type):
    ...
    while pos < len(token_list):
        ...
        if t['label'] == "python":
            ...
            parent.add(new_node)
            pos += 1
            parse_result = parse_template(token_list, pos, new_node.children['True'], "if")
            pos = parse_result[1]
            t = token_list[pos]
            if t['label'] == 'else':
                pos += 1
                parse_result = parse_template(token_list, pos, new_node.children['False'], "else")
                pos = parse_result[1]
        elif t['label'] == "else":
            if parent_type == 'if':
                return (parent,pos)
            else:
                raise TemplateException("Unexpected 'else' tag. Parent is {!r}".format(parent_type))
        elif t['label'] == "end if":
